...
check('getOneTimePriceUSD returns the documented $7 one-time price', () => {
...
check('validateCard uses the real runtime clock for expiry — a card expiring next month is still valid today', () => {
  const now = new Date();
  const future = new Date(now.getFullYear(), now.getMonth() + 2, 1);
  const mm = String(future.getMonth() + 1).padStart(2, '0');
  const yy = String(future.getFullYear() % 100).padStart(2, '0');
  const { valid, errors } = validateCard({ number: '4242424242424242', expiry: `${mm}/${yy}`, cvc: '123' });
